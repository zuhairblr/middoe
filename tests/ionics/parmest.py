...
def solve_modeli(t, y0, tii, tvi, theta):
    ...
    uncvar = {                                   # Uncontrollable variables
        ...
        'slurry_volume': 5e-1,                    # Liquid volume (l)
    ...
    C_proton_mol_l = 10 ** (-11.53)  # H+ initial concentration (mol/l)
    ...
    k5f_fixed = const['k5f_fixed']                 # l/(mol·s) for R4 forward
    T_ref = const['T_ref']                         # K
    ...
    def odes(tt, y):
        ...
        c_std = 1  # standard state concentration mol/m³ (equivalent to 1 mol/L)
        r_R1 = k_R1f * C_CO2_aq_mol_l - (k_R1f / Keq_R1) * ((C_carbonate_mol_l / c_std) * gamma_HCO3) * (
                    (C_proton_mol_l / c_std) * gamma_H)
        r_R2 = k_R2f * ((C_carbonate_mol_l / c_std) * gamma_HCO3) - (k_R2f / Keq_R2) * (
                    (C_bicarbonate_mol_l / c_std) * gamma_CO3) * ((C_proton_mol_l / c_std) * gamma_H)
        r_R5 = k_R5f * C_CO2_aq_mol_l * ((C_OH_bulk_mol_l / c_std) * gamma_OH) - (k_R5f / Keq_R5) * (
                    (C_carbonate_mol_l / c_std) * gamma_HCO3)
        r_R3 = k_R3f * (((C_CH_bulk_mol_l / c_std) * gamma_Ca) * (
                    (C_bicarbonate_mol_l / c_std) * gamma_CO3) / Ksp_CaCO3 - 1)
        r_recomb = k5f_fixed * ((C_proton_mol_l / c_std) * gamma_H) * (
                    (C_OH_bulk_mol_l / c_std) * gamma_OH) - k5f_fixed / Keq_R4
        ...
        dC_OH_bulk_mol_l_dt =  kO * solid_liquid_total_surface * (
                C_OH_void_mol_l - C_OH_bulk_mol_l) - r_recomb - r_R5
...
def scaled_objective_theta(theta, t, tii, tvi,
    ...
    try:
        ...
        print(f'times={exp_times_C_proton}')
        print (f'exp_values={exp_values_C_proton}')
        print(f'model_vals={model_vals}')

        # Edge cases: if ss_tot is zero (all exp values same), return penalty
        if ss_tot == 0:
            return penalty

        r2 = 1 - ss_res / ss_tot

        # We minimize objective, so return 1 - R2
        obj = 1 - r2

        # Prevent negative due to floating error
        if obj < 0:
            obj = 0.0
...
#     #     maxiter=10,
#     #     disp=True,
#     #     polish=True,  # optional: polish final solution by local optimization
#     #     tol=1e-6
#     # )
#
#     theta_opt = res.x
