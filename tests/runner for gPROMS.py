...
#     te_array = np.array(te) if not isinstance(te, np.ndarray) else te
#     reactor_temperature = np.interp(t, te_array, tvi['T'])
#     partial_pressure = np.interp(t, te_array, tvi['P'])
#
#     # Clamp states
...
#
#     # Unpack tii
#     particle_size = tii['aps'] * 1e-6  # μm to m
#     slurry_density = tii['slr']        # kg solid / L liquid
#
#
#
#     # Constants
...
#     particle_volumetric_surface = 6e6 / (tii['aps'])  # m²/m³
#
#     # Solid holdup at t=0
...
def main():
    ...
    modelling_settings = { # Settings related to the rival models and their parameters
        ...
        'active_models': ['f20'], # Active solvers (rival models) to be used in the experiment
